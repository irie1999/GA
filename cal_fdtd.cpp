...
double **cal_fdtd(double beta, double h_prime, int t){
   ...
   //input(Ne, t, s);
    ...
    // for(int j = 100; j <= Nth - PML_L; j++){
    //   ofs << j * R0 * dth * 1e-3 << " " << std::abs( Er0[j] ) << std::endl;
    //       //<< std::arg( Er0[j] ) << "\n";
    // }
    // ofs.close();
    for(int j = 100; j <= Nth - PML_L; j++){
      Ei_tm[t][j] = std::abs( Er0[j] );
    }
  ...
  double time_GA[3];
  time_GA[0] = 6.1667, time_GA[1] = 6.313, time_GA[2] = 6.5;
  double **Si_tm = allocate_memory2d(3,  Nth, 0.0);

    for(int j = 100; j <= Nth - PML_L; j++){
      Si_tm[t][j] = (Ei_tm[t][j] - Ei_tm[t-1][j]) / (time_GA[t] - time_GA[t-1]);
    }
  

  // std::ofstream ofs("../GA/data/" "GA_k_Si_tm_1");
  // for(int j = 100; j <= Nth - PML_L; j++){
  //   ofs << Si_tm[1][j] << std::endl;
  // }
  // ofs.close();

  // std::ofstream ofs1("../GA/data/" "GA_k_Si_tm_2");
  // for(int j = 100; j <= Nth - PML_L; j++){
  //   ofs1 << Si_tm[2][j] << std::endl;
  // }
  // ofs1.close();
